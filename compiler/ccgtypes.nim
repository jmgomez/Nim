...
type
  TypeDescKind = enum
    dkParam #skParam
    dkRefParam #skVar and byref
    dkField #skField
    dkResult #skResult
    dkConst #skConst
    dkType #skType (type section only)
    dkTemp #skTemp
    dkForVar #skForVar
    dkLet #dkLet
...
proc descKindFromSymKind(kind: TSymKind): TypeDescKind =
  case kind
  of skParam: result = dkParam
  of skVar: result = dkRefParam
  of skField: result = dkField
  of skResult: result = dkResult
  of skConst: result = dkConst
  of skType: result = dkType
  of skTemp: result = dkTemp
  of skForVar: result = dkForVar
  of skLet: result = dkLet
  else: 
    internalError("descKindFromSymKind unsupported symKind:" & $kind)

proc symKindFromDescKind(kind: TypeDescKind): TSymKind =
  case kind
  of dkParam: skParam
  of dkRefParam: skVar
  of dkField: skField
  of dkResult: skResult
  of dkConst: skConst
  of dkType: skType
  of dkTemp: skTemp
  of dkForVar: skForVar
  of dkLet: skLet
...
proc getTypeDescAux(m: BModule; origTyp: PType, check: var IntSet; kind: TypeDescKind): Rope =
  ...
  of tyRef, tyPtr, tyVar, tyLent:
    ...
    if mapType(m.config, t, symKindFromDescKind(kind)) == ctPtrToArray and (etB.kind != tyOpenArray or kind == dkParam):
      if etB.kind == tySet:
